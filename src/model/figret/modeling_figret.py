...
class FigretTeModel(nn.Module):
    def __init__(self, config):
        ...
        # self.model = FigretNetWork(self.input_dim, self.output_dim, self.num_layers).double()
        self.model = FigretNetWork(self.input_dim, self.output_dim, self.num_layers)
    ...
    def loss(self, y_pred_batch, te_inputs, optimal_mlu):
        ...
        for i in range(batch_size):
            ...
            tm_hist_std = torch.std(hist, dim=-1)  # Shape: (batch_size, num_pairs)
            ...
            max_sensitivity = max_sensitivity.repeat_interleave(
                self.num_paths, 
                dim=0
            )

            weight_max_sensitivity = max_sensitivity.mul(tm_hist_std) #(num_commodities,)
            ...
            loss = 1.0 - max_cong if max_cong.item() == 0.0 else \
                    max_cong / max_cong.item() + self.alpha * sum_wm_sens / sum_wm_sens.item()
