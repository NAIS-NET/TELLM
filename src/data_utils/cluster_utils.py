...
class Cluster_Info:
    def __init__(self, sp: Read_Snapshot, props, cluster, dataset_dir):
        self.sp = sp
    ...
    def compute_ksp_paths(self, k, pairs):
        ...
        filepath = f"{self.dataset_dir}/topologies/paths_dict/{self.props.topo}_{k}_paths_dict_cluster_{self.cluster}.pkl"
        try:
            ...
            for src, dst in tqdm.tqdm(pairs, desc="Computing k-shortest paths", unit="pair"):
    ...
    def get_padded_edge_ids_per_path(self, pij, edges_map) -> torch.Tensor:
        ...
        os.makedirs(f"{self.dataset_dir}/topologies/padded_edge_ids_per_path", exist_ok=True)
        filepath = f"{self.dataset_dir}/topologies/padded_edge_ids_per_path/{self.props.topo}_{self.props.num_paths_per_pair}_paths_cluster_{self.cluster}_padded_edge_ids_per_path.pkl"
        try:
    ...
    def get_paths_to_edges_matrix(self, pij: dict) -> torch.sparse_coo_tensor:
        ...
        filepath = f"{self.dataset_dir}/topologies/paths/{self.props.topo}_{self.props.num_paths_per_pair}_paths_cluster_{self.cluster}.pkl"
        try:
    ...
    def get_flow_matrix(self):
        ...
        path_dict = self.compute_ksp_paths(self.props.num_paths_per_pair, self.sp.pairs)
        pairs = self.sp.pairs

        # edge nodes' degree, index lookup
        edge2idx_dict = self.edges_map
        # edge2idx_dict = {edge: idx for idx, edge in enumerate(self.sp.graph.edges)}
        node2degree_dict = {}

        edge_num = len(self.sp.graph.edges)
        src = []
        dst = []
        path_i = 0
        for pair in pairs:
            for path in path_dict[pair[0], pair[1]]:
                for edge in path:
                    # src are path_nodes, dst are edge_nodes
                    src.append(path_i + edge_num)  # path index starts from edge_num
                    dst.append(edge2idx_dict[edge])
        ...
        # # build edge_index
        # src, dst, path_i = [], [], 0
        # for s in range(len(self.sp.graph)):
        #     for t in range(len(self.sp.graph)):
        #         if s == t:
        #             continue
        #         if (s, t) not in path_dict:
        #             continue
        #         for k in path_dict[(s, t)]:
        #             path = path_dict[(s, t)][k]   # 现在是edges tuple的list
        #             for (u, v) in path:
