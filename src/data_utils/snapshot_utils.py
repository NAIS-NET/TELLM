...
class Read_Snapshot:
    def __init__(self, props, topology_filename, pairs_filename, tm_filename, dataset_dir):
        ...
        self._node_features_norm = self.get_node_features_norm()

    def read_pairs_from_pkl(self, pairs_filename):
        file = open(f"{self.dataset_dir}/pairs/{self.topo}/{pairs_filename}", "rb")
    ...
    def read_graph_from_json(self, topo: str, topology_filename):
        with open(f"{self.dataset_dir}/topologies/{self.topo}/{topology_filename}") as f:
            data = json.load(f)
        
        graph = nx.readwrite.json_graph.node_link_graph(data, edges='links')
    ...
    def get_node_features_norm(self):
        degrees = dict(self.graph.in_degree())
        degrees = torch.tensor(list(degrees.values()), dtype=torch.float32)
        degrees = degrees.reshape(-1, 1)
        edge_index = self.get_edge_index()
        ...
        mask = torch.where(self.capacities == 0)
        self.capacities[mask] += 1e-4
        normalized_capacities = self.capacities / self.capacities.max()
        
        cap_sum_list = []
        for node in self.graph.nodes():
            node_id = self.node_ids_map[node]
            indices = (edge_index[0] == node_id).nonzero()
            cap_sum = torch.sum(normalized_capacities[indices])
            cap_sum_list.append(cap_sum)
        cap_sum_list = torch.tensor(cap_sum_list, dtype=torch.float32)
        cap_sum_list = cap_sum_list.reshape(-1, 1)
        node_features = torch.cat((degrees, cap_sum_list), dim=1)
    ...
    def read_tms(self, tm_filename):
        file = open(f"{self.dataset_dir}/traffic_matrices/{self.topo}/{tm_filename}", 'rb')
        tm = pickle.load(file)
        tm = tm.reshape(-1, 1)
    ...
    def read_tms_pred(self, tm_filename):
        file = open(f"{self.dataset_dir}/traffic_matrices/{self.topo}_pred/{tm_filename}", 'rb')
        tm = pickle.load(file)
        tm = tm.reshape(-1, 1)
        tm = tm.astype(np.float32)
        tm = np.repeat(tm, repeats=self.props.num_paths_per_pair, axis=0)
